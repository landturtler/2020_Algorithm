...
배운 점: 특정 방향으로 갈 때 비용이 다른 경우, visited[] 배열에 특정 방향을 고려해서 3차원으로 저장할 것
        한 문제만 시간초과 날 경우, 범위 잘 보기 ( < 이랑 <= 차이 때문에 시간초과 날 수도 있다..)
소요 시간 : 50분
'''
...
def solution(board):
    ...
    N = len(board)
    dir = [ [-1, 0], [0, 1], [1, 0], [0, -1]] 
   #visited[i][j][0] : (i,j)에 수직방향으로 도달할 수있는 최소 비용  / visited[i][j][1] : (i,j)에 수평방향으로 도달할 수 있는 최소 비용 
    ...
    while q:
        ...
        for k in range(4):
            ...
            if board[nx][ny] == 1 or visited[nx][ny][nd % 2] <= nc:
                continue
